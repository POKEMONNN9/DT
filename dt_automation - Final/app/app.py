...
@app.route('/test/domaintools', methods=['GET'])
def test_domaintools_connection():
    """Test DomainTools API connection"""
    try:
        from domaintools import test_domaintools_connection
        success, message = test_domaintools_connection()
        if success:
            return jsonify({'success': True, 'message': message})
        else:
...
@app.route('/test/phishlabs', methods=['GET'])
def test_phishlabs_connection():
    """Test PhishLabs API connection"""
    try:
        from phishlabs import test_phishlabs_connection
        success, message = test_phishlabs_connection()
        if success:
            return jsonify({'success': True, 'message': message})
        else:
            return jsonify({'success': False, 'message': message}), 400
