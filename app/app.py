...
class ThreatDashboard:
        ...
        try:
            ...
            WITH attribution_analysis AS (
                SELECT 
                    ...
                    CASE WHEN b.bot_name IS NOT NULL THEN 1 ELSE 0 END as has_bot_detection,
                    (CASE WHEN th.name IS NOT NULL THEN 1 ELSE 0 END +
                     ...
                     CASE WHEN b.bot_name IS NOT NULL THEN 1 ELSE 0 END) as attribution_score
                FROM phishlabs_case_data_incidents i
            ...
            SELECT 
                ...
                ROUND((SUM(has_threat_actor) * 100.0 / COUNT(*)), 1) as threat_actor_coverage_pct,
                ROUND((SUM(has_kit_family) * 100.0 / COUNT(*)), 1) as kit_family_coverage_pct,
                ROUND((SUM(has_whois) * 100.0 / COUNT(*)), 1) as whois_coverage_pct
            ...
            if isinstance(result, dict) and 'error' in result:
                return {
                    "threat_actor_coverage_pct": 0,
                    "kit_family_coverage_pct": 0,
                    "whois_coverage_pct": 0,
    ...
    def get_top_threat_actors(self, date_filter="today", campaign_filter="all", start_date=None, end_date=None):
        ...
        try:
            ...
            actor_query = f"""
            SELECT TOP 10
                th.name as threat_actor_handle,
                COUNT(DISTINCT i.case_number) as total_cases,
                COUNT(DISTINCT u.domain) as unique_domains,
                COUNT(DISTINCT u.ip_address) as unique_ips,
                COUNT(DISTINCT u.host_country) as countries_targeted,
                STRING_AGG(n.threat_family, ',') as kit_families_used,
            ...
            for actor in actors:
                cases = actor.get('total_cases', 0)
                domains = actor.get('unique_domains', 0)
                countries = actor.get('countries_targeted', 0)
                duration = actor.get('campaign_duration_days', 0)
    ...
    def get_infrastructure_patterns(self, date_filter="today", campaign_filter="all", start_date=None, end_date=None):
        ...
        try:
            ...
            return {
                "top_tlds": tlds if not isinstance(tlds, dict) else [],
                "host_countries": countries if not isinstance(countries, dict) else [],
                "hosting_providers": isps if not isinstance(isps, dict) else []
            }
            
        except Exception as e:
            logger.error(f"Error in get_infrastructure_patterns: {e}")
            return {"top_tlds": [], "host_countries": [], "hosting_providers": []}
        ...
        for campaign_name, campaign_data in all_campaigns_data.items():
            for identifier in campaign_data.get('identifiers', []):
                if isinstance(identifier, dict):
                    ...
                    if field == 'case_number':
                        case_to_campaign[value] = campaign_name
                    elif field == 'infrid':
                        infrid_to_campaign[value] = campaign_name
                    elif field == 'incident_id':
                        incident_to_campaign[value] = campaign_name
